...
async def main():
    ...
    print('the startup speed of the windows depend on the line speed and availability/load on the servers')
    ...
    nowsecure_pages = [tab for tab in driver.tabs if 'nowsecure' in (tab.url or '')]
    ...
    b365pages = [tab for tab in driver.tabs if 'bet365' in (tab.url or '')]
...
async def scroll_task(tab):
    ...
    [
        ...
        or print(f'tab {tab} scrolling down: {n}')
...
async def move_circle(tab, x=0):
    ...
    for coord in uc.util.circle(*center, radius=100, num=1050, dir_=x):
        new_left, new_top = int(coord[0]), int(coord[1])
        await tab.set_window_size(new_left, new_top)

    for coord in uc.util.circle(*center, radius=250, num=500, dir_=x):
...
async def flash_spans(tab, i) -> None:
    logger.info(f'flashing spans. {i=} , {tab=}, {tab.url=}')
